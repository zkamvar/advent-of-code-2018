...
#     though F was available earlier, steps B and D are now also available, and
#     B is the first alphabetically of the three.  After that, only D and F are
#     available. E is not available because only some of its prerequisites are
#     complete. Therefore, D is completed next.  F is the only choice, so it is
#     done next.  Finally, E is completed.
#
...
cat(paste(find_order(zstep), collapse = ""), "\n")
